...
void analyze(const char* filename){
    ...
    std::string outfile_name = "specific_heat_" + filename_noext + ".csv";
    std::ofstream fout(outfile_name);
    std::cout << "Calculated specific heat data for " << std::string(filename);
    std::cout << "\nWriting data to " << outfile_name << "...";
    fout << "T,Specific heat\n";
